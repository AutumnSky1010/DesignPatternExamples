...
* オブジェクトを生成するFactory Methodが定義された抽象クラスないしI/Fを継承したサブクラスに任せる。
* そうすることで、アプリケーションに特化した処理を記述しているのにもかかわらず再利用が簡単になる。
* 
* 【メリット】
* ・生成するインスタンスの切り替えが行いやすくなる。(動的に生成する場合など)
...
namespace DesignPatternExamples.Creational.FactoryMethod.Domain
{
    ...
    /**
     * 抽象クラスを用いるとTemplate Methodパターンと組み合わせることができる。
     */
    /// <summary>
